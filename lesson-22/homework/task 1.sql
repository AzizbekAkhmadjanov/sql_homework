...
--    SELECT
--        *,
--        AVG(total_spent) OVER (PARTITION BY region) AS avg_region_spending
--    FROM customer_spending
--) AS spending_data
--WHERE total_spent > avg_region_spending
--ORDER BY region, total_spent DESC;

--------------------------------------------------------------------------------
--16. Rank customers based on their total spending (total_amount) within 
--each region. If multiple customers have the same spending, they should receive 
--the same rank.
...
--------------------------------------------------------------------------------
--17. Calculate the running total (cumulative_sales) of total_amount for each 
--		customer_id, ordered by order_date. 
...
--------------------------------------------------------------------------------
--18. Calculate the sales growth rate (growth_rate) for each month compared to 
--		the previous month.
...
--    ROUND(
--        ( (monthly_total - LAG(monthly_total) OVER (ORDER BY sales_year, sales_month)) 
--          / NULLIF(LAG(monthly_total) OVER (ORDER BY sales_year, sales_month), 0)
--        ) * 100, 2
--    ) AS growth_rate
--FROM monthly_sales
...
--------------------------------------------------------------------------------
--19. Identify customers whose total_amount is higher than their last order''s 
--		total_amount.(Table sales_data)
...
--------------------------------------------------------------------------------
--21. In this puzzle you have to find the sum of val1 and val2 for each group and 
--put that value at the beginning of the group in the new column. 
--The challenge here is to do this in a single select. For more details please 
...
--------------------------------------------------------------------------------
--22. Here you have to sum up the value of the cost column based on the values 
--of Id. For Quantity if values are different then we have to add those values.
...
--------------------------------------------------------------------------------
--23. From following set of integers, write an SQL statement to determine 
...
---- Handle the gap before the first seat (if first seat > 1)
--SELECT 
--    1 AS Gap_Start,
--    (SELECT MIN(SeatNumber) - 1 FROM Seats) AS Gap_End
