...
--------------------------------------------------------------------------------
--2. Write a query to rank products based on the total quantity sold. give the 
--		same rank for the same amounts without skipping numbers.
...
--------------------------------------------------------------------------------
--5. Write a query to display each sale's amount along with the previous sale 
--		amount in the order of SaleDate.
...
--------------------------------------------------------------------------------
--6. Write a query to identify sales amounts that are greater than the previous 
--			sale's amount
...
--------------------------------------------------------------------------------
--7. Write a query to calculate the difference in sale amount from the previous 
--			sale for every product
--SELECT 
--    SaleID,
--    ProductName,
--    SaleDate,
...
--------------------------------------------------------------------------------
--8. Write a query to compare the current sale amount with the next sale amount 
...
--    LEAD(SaleAmount) OVER (ORDER BY SaleDate) AS NextSaleAmount,
--    CASE 
--        WHEN LEAD(SaleAmount) OVER (ORDER BY SaleDate) IS NULL 
--            THEN NULL
...
--------------------------------------------------------------------------------
--9. Write a query to calculate the ratio of the current sale amount to the 
--		previous sale amount within the same product.
...
--        ) IS NULL 
--            THEN NULL
--        ELSE SaleAmount * 1.0 / LAG(SaleAmount) OVER (
--            PARTITION BY ProductName 
--            ORDER BY SaleDate
--        )
--    END AS AmountRatio
...
--------------------------------------------------------------------------------
--10. Write a query to calculate the difference in sale amount from the very 
--		first sale of that product.
...
--        PARTITION BY ProductName 
--        ORDER BY SaleDate
--    ) AS DifferenceFromFirst
--FROM ProductSales;

--------------------------------------------------------------------------------
--12. Write a query to find sales that have been increasing continuously for a 
--	product (i.e., each sale amount is greater than the previous sale amount for 
--	that product).
...
--------------------------------------------------------------------------------
--12. Write a query to calculate a "closing balance"(running total) for sales 
--amounts which adds the current sale amount to a running total of previous sales.
...
--------------------------------------------------------------------------------
--22. Calculate the Moving Average Salary Over 3 Employees 
--		(Including Current, Previous, and Next)
